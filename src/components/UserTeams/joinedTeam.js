...
function JoinedTeam() {
  ...
  const 팀나가기 = (teamId) => {
    fetch(
      ...
      }/teams/deleteMember?teamId=${teamId}&userId=${localStorage.getItem(
        "userId"
      )}`,
      {
        method: "POST",
      ...
      .then((obj) => {
        ...
        if (obj.message != "팀장은 팀을 나갈 수 없습니다")
          setJoinedTeams(
            joinedTeams.filter((data) => {
  ...
  const 채팅 = (data) => {
    window.open(
      `/chat?waitingId=${data.teamId}&nickname=${localStorage.getItem(
        "nickname"
      )}&mode=room`,
      "_blank",
          ...
          <div key={data.teamId} className="joined-team-card-ryu">
            ...
            <div className="joinedTeam-card-bottom">
              ...
              <div className="joinedTeam-btn-group">
                <button className="chatBtn" onClick={() => 채팅(data)}>
                  채팅
                </button>
                <button
                  className="exitBtn"
                  onClick={() => 팀나가기(data.teamId)}
                >
                  팀나가기
                </button>
              </div>
